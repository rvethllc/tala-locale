...
EXTENDED_LOCALE_MAP: dict[str, ExtendedLocale] = {
    ...
    "CI": ExtendedLocale(
        "CFA", False, ",", " ", 0.18, "TVA", "%d/%m/%Y", False, 0, "fr-CI", ("fr-CI", "dyu-CI")
    ),
    ...
    "LR": ExtendedLocale(
        "L$", True, ".", ",", 0.10, "GST", "%d/%m/%Y", False, 0, "en-LR", ("en-LR",)
    ),
    # Sierra Leone: English + Krio (Creole lingua franca)
    "SL": ExtendedLocale(
        "Le", True, ".", ",", 0.15, "GST", "%d/%m/%Y", False, 0, "en-SL", ("en-SL", "kri-SL")
    ),
    # Gambia: English only primary
    "GM": ExtendedLocale(
        "D", True, ".", ",", 0.15, "VAT", "%d/%m/%Y", False, 0, "en-GM", ("en-GM", "wo-GM")
    ),
    # Mali: French + Bambara (lingua franca)
    "ML": ExtendedLocale(
        "CFA", False, ",", " ", 0.18, "TVA", "%d/%m/%Y", False, 0, "fr-ML", ("fr-ML", "bm-ML")
    ),
    ...
    "GW": ExtendedLocale(
        "CFA", False, ",", " ", 0.17, "TVA", "%d/%m/%Y", False, 0, "pt-GW", ("pt-GW", "pov-GW")
    ),
    # Cape Verde: Portuguese + Cape Verdean Creole
    "CV": ExtendedLocale(
        "$", True, ",", ".", 0.15, "IVA", "%d/%m/%Y", False, 0, "pt-CV", ("pt-CV", "kea-CV")
    ),
    # Mauritania: Arabic + French + Pulaar
    "MR": ExtendedLocale(
        "UM", True, ",", " ", 0.16, "TVA", "%d/%m/%Y", True, 0, "ar-MR", ("ar-MR", "fr-MR", "ff-MR")
    ),
    # ── East Africa ──────────────────────────────────────────────────────────
    ...
    "TZ": ExtendedLocale(
        "TSh", True, ".", ",", 0.18, "VAT", "%d/%m/%Y", False, 0, "sw-TZ", ("sw-TZ", "en-TZ")
    ),
    ...
    "SO": ExtendedLocale(
        "Sh.So.", True, ".", ",", 0.0, "", "%d/%m/%Y", False, 0, "so-SO", ("so-SO", "ar-SO")
    ),
    ...
    "ER": ExtendedLocale(
        "Nfk", True, ".", ",", 0.0, "", "%d/%m/%Y", False, 0, "ti-ER", ("ti-ER", "ar-ER", "en-ER")
    ),
    ...
    "EG": ExtendedLocale(
        "E£", True, ".", ",", 0.14, "VAT", "%d/%m/%Y", True, 6, "ar-EG", ("ar-EG", "en-EG")
    ),
    ...
    "TN": ExtendedLocale(
        "DT", False, ",", ".", 0.19, "TVA", "%d/%m/%Y", True, 0, "ar-TN", ("ar-TN", "fr-TN")
    ),
    # Libya: Libyan Arabic (distinct dialect, ar-LY) — minimal French unlike Maghreb
    "LY": ExtendedLocale("LD", True, ".", ",", 0.0, "", "%d/%m/%Y", True, 6, "ar-LY", ("ar-LY",)),
    # Sudan: Sudanese Arabic + Nile Nubian + Beja
    "SD": ExtendedLocale(
        "SDG", True, ".", ",", 0.17, "VAT", "%d/%m/%Y", True, 6, "ar-SD", ("ar-SD", "en-SD")
    ),
    # ── Southern Africa ──────────────────────────────────────────────────────
    ...
    "BW": ExtendedLocale(
        "P", True, ".", ",", 0.12, "VAT", "%d/%m/%Y", False, 0, "en-BW", ("en-BW", "tn-BW")
    ),
    # Lesotho: Sotho + English
    "LS": ExtendedLocale(
        "M", True, ".", ",", 0.15, "VAT", "%d/%m/%Y", False, 0, "en-LS", ("en-LS", "st-LS")
    ),
    # Eswatini: Swati + English
    "SZ": ExtendedLocale(
        "E", True, ".", ",", 0.15, "VAT", "%d/%m/%Y", False, 0, "en-SZ", ("en-SZ", "ss-SZ")
    ),
    ...
    "MW": ExtendedLocale(
        "MK", True, ".", ",", 0.165, "VAT", "%d/%m/%Y", False, 0, "en-MW", ("en-MW", "ny-MW")
    ),
    ...
    "CF": ExtendedLocale(
        "CFA", False, ",", " ", 0.19, "TVA", "%d/%m/%Y", False, 0, "fr-CF", ("fr-CF", "sg-CF")
    ),
    # Chad: French + Arabic + Sara
    "TD": ExtendedLocale(
        "CFA", False, ",", " ", 0.18, "TVA", "%d/%m/%Y", False, 0, "fr-TD", ("fr-TD", "ar-TD")
    ),
    # Gabon: French only dominant
    "GA": ExtendedLocale(
        "CFA", False, ",", " ", 0.18, "TVA", "%d/%m/%Y", False, 0, "fr-GA", ("fr-GA",)
    ),
    # Republic of Congo: French + Lingala + Kituba
    "CG": ExtendedLocale(
        "CFA", False, ",", " ", 0.18, "TVA", "%d/%m/%Y", False, 0, "fr-CG", ("fr-CG", "ln-CG")
    ),
    ...
    "ST": ExtendedLocale("Db", True, ",", ".", 0.0, "", "%d/%m/%Y", False, 0, "pt-ST", ("pt-ST",)),
    # ── Indian Ocean ─────────────────────────────────────────────────────────
    # Madagascar: Malagasy + French
    "MG": ExtendedLocale(
        "Ar", False, ",", " ", 0.20, "TVA", "%d/%m/%Y", False, 0, "mg-MG", ("mg-MG", "fr-MG")
    ),
    ...
    "GB": ExtendedLocale(
        "£", True, ".", ",", 0.20, "VAT", "%d/%m/%Y", False, 0, "en-GB", ("en-GB", "cy-GB", "gd-GB")
    ),
    # France: French — fr-FR is different from fr-CA, fr-BE, fr-CH
    "FR": ExtendedLocale(
        "€", False, ",", " ", 0.20, "TVA", "%d/%m/%Y", False, 0, "fr-FR", ("fr-FR",)
    ),
    # Germany: German
    "DE": ExtendedLocale(
        "€", False, ",", ".", 0.19, "MwSt.", "%d.%m.%Y", False, 0, "de-DE", ("de-DE",)
    ),
    # Netherlands: Dutch + Frisian
    "NL": ExtendedLocale(
        "€", True, ",", ".", 0.21, "BTW", "%d-%m-%Y", False, 0, "nl-NL", ("nl-NL", "fy-NL")
    ),
    ...
    # Italy: Italian
    "IT": ExtendedLocale(
        "€", False, ",", ".", 0.22, "IVA", "%d/%m/%Y", False, 0, "it-IT", ("it-IT",)
    ),
    ...
    # Austria: German
    "AT": ExtendedLocale(
        "€", False, ",", ".", 0.20, "MwSt.", "%d.%m.%Y", False, 0, "de-AT", ("de-AT",)
    ),
    ...
    # Portugal
    "PT": ExtendedLocale(
        "€", False, ",", ".", 0.23, "IVA", "%d/%m/%Y", False, 0, "pt-PT", ("pt-PT",)
    ),
    # Ireland: English + Irish (Gaelic)
    "IE": ExtendedLocale(
        "€", True, ".", ",", 0.23, "VAT", "%d/%m/%Y", False, 0, "en-IE", ("en-IE", "ga-IE")
    ),
    ...
    # Monaco: French
    "MC": ExtendedLocale(
        "€", False, ",", " ", 0.20, "TVA", "%d/%m/%Y", False, 0, "fr-MC", ("fr-MC",)
    ),
    # San Marino: Italian
    "SM": ExtendedLocale("€", False, ",", ".", 0.0, "", "%d/%m/%Y", False, 0, "it-SM", ("it-SM",)),
    # Vatican: Italian + Latin
    "VA": ExtendedLocale(
        "€", False, ",", ".", 0.0, "", "%d/%m/%Y", False, 0, "it-VA", ("it-VA", "la-VA")
    ),
    # Liechtenstein: German
    "LI": ExtendedLocale(
        "CHF", True, ".", "'", 0.081, "MWST", "%d.%m.%Y", False, 0, "de-LI", ("de-LI",)
    ),
    # ── Northern Europe ──────────────────────────────────────────────────────
    "SE": ExtendedLocale(
        "kr", False, ",", " ", 0.25, "Moms", "%Y-%m-%d", False, 0, "sv-SE", ("sv-SE",)
    ),
    ...
    "DK": ExtendedLocale(
        "kr", True, ",", ".", 0.25, "Moms", "%d/%m/%Y", False, 0, "da-DK", ("da-DK",)
    ),
    # Finland: Finnish + Swedish (both official)
    "FI": ExtendedLocale(
        "€", False, ",", " ", 0.255, "ALV", "%d.%m.%Y", False, 0, "fi-FI", ("fi-FI", "sv-FI")
    ),
    # Iceland
    "IS": ExtendedLocale(
        "kr", False, ",", ".", 0.24, "VSK", "%d/%m/%Y", False, 0, "is-IS", ("is-IS",)
    ),
    # Faroe Islands: Faroese + Danish
    "FO": ExtendedLocale(
        "kr", True, ",", ".", 0.25, "MVG", "%d/%m/%Y", False, 0, "fo-FO", ("fo-FO", "da-FO")
    ),
    # Greenland: Greenlandic/Kalaallisut + Danish
    "GL": ExtendedLocale(
        "kr", True, ",", ".", 0.0, "", "%d/%m/%Y", False, 0, "kl-GL", ("kl-GL", "da-GL")
    ),
    # ── Southern Europe ──────────────────────────────────────────────────────
    # Greece: Greek
    "GR": ExtendedLocale(
        "€", False, ",", ".", 0.24, "ΦΠΑ", "%d/%m/%Y", False, 0, "el-GR", ("el-GR",)
    ),
    # Cyprus: Greek + Turkish (both official)
    "CY": ExtendedLocale(
        "€", True, ".", ",", 0.19, "ΦΠΑ", "%d/%m/%Y", False, 0, "el-CY", ("el-CY", "tr-CY")
    ),
    # Malta: Maltese + English
    "MT": ExtendedLocale(
        "€", True, ".", ",", 0.18, "VAT", "%d/%m/%Y", False, 0, "mt-MT", ("mt-MT", "en-MT")
    ),
    # ── Eastern Europe ───────────────────────────────────────────────────────
    "RU": ExtendedLocale(
        "₽", False, ",", " ", 0.20, "НДС", "%d.%m.%Y", False, 0, "ru-RU", ("ru-RU",)
    ),
    # Ukraine: Ukrainian + Russian
    "UA": ExtendedLocale(
        "₴", False, ",", " ", 0.20, "ПДВ", "%d.%m.%Y", False, 0, "uk-UA", ("uk-UA", "ru-UA")
    ),
    "PL": ExtendedLocale(
        "zł", False, ",", " ", 0.23, "VAT", "%d.%m.%Y", False, 0, "pl-PL", ("pl-PL",)
    ),
    "HU": ExtendedLocale(
        "Ft", False, ",", " ", 0.27, "ÁFA", "%Y.%m.%d", False, 0, "hu-HU", ("hu-HU",)
    ),
    "CZ": ExtendedLocale(
        "Kč", False, ",", " ", 0.21, "DPH", "%d.%m.%Y", False, 0, "cs-CZ", ("cs-CZ", "sk-CZ")
    ),
    "SK": ExtendedLocale(
        "€",
        False,
    ...
    "RO": ExtendedLocale(
        "lei", False, ",", ".", 0.19, "TVA", "%d.%m.%Y", False, 0, "ro-RO", ("ro-RO", "hu-RO")
    ),
    "BG": ExtendedLocale(
        "лв", False, ",", " ", 0.20, "ДДС", "%d.%m.%Y", False, 0, "bg-BG", ("bg-BG",)
    ),
    "LT": ExtendedLocale(
        "€", False, ",", " ", 0.21, "PVM", "%Y-%m-%d", False, 0, "lt-LT", ("lt-LT",)
    ),
    "LV": ExtendedLocale(
        "€", False, ",", " ", 0.21, "PVN", "%d.%m.%Y", False, 0, "lv-LV", ("lv-LV", "ru-LV")
    ),
    "EE": ExtendedLocale(
        "€", False, ",", " ", 0.22, "KM", "%d.%m.%Y", False, 0, "et-EE", ("et-EE", "ru-EE")
    ),
    # Belarus: Belarusian + Russian (both official)
    "BY": ExtendedLocale(
        "Br", False, ",", " ", 0.20, "НДС", "%d.%m.%Y", False, 0, "be-BY", ("be-BY", "ru-BY")
    ),
    # Moldova: Romanian + Russian
    "MD": ExtendedLocale(
        "L", False, ",", ".", 0.20, "TVA", "%d.%m.%Y", False, 0, "ro-MD", ("ro-MD", "ru-MD")
    ),
    "SI": ExtendedLocale(
        "€", False, ",", ".", 0.22, "DDV", "%d.%m.%Y", False, 0, "sl-SI", ("sl-SI",)
    ),
    "HR": ExtendedLocale(
        "€", False, ",", ".", 0.25, "PDV", "%d.%m.%Y", False, 0, "hr-HR", ("hr-HR",)
    ),
    ...
    "RS": ExtendedLocale(
        "RSD", False, ",", ".", 0.20, "PDV", "%d.%m.%Y", False, 0, "sr-RS", ("sr-RS", "hu-RS")
    ),
    "ME": ExtendedLocale(
        "€", False, ",", ".", 0.21, "PDV", "%d.%m.%Y", False, 0, "sr-ME", ("sr-ME", "sq-ME")
    ),
    # Kosovo: Albanian + Serbian (both official)
    "XK": ExtendedLocale(
        "€", True, ",", ".", 0.18, "TVSH", "%d.%m.%Y", False, 0, "sq-XK", ("sq-XK", "sr-XK")
    ),
    # North Macedonia: Macedonian + Albanian
    "MK": ExtendedLocale(
        "den", False, ",", ".", 0.18, "ДДВ", "%d.%m.%Y", False, 0, "mk-MK", ("mk-MK", "sq-MK")
    ),
    "AL": ExtendedLocale(
        "L", False, ",", ".", 0.20, "TVSH", "%d.%m.%Y", False, 0, "sq-AL", ("sq-AL",)
    ),
    # ── Middle East ──────────────────────────────────────────────────────────
    # UAE: Gulf Arabic (ar-AE) + English widely used in business
    "AE": ExtendedLocale(
        "AED", True, ".", ",", 0.05, "VAT", "%d/%m/%Y", True, 0, "ar-AE", ("ar-AE", "en-AE")
    ),
    # Saudi Arabia: Gulf Arabic (ar-SA) — formal MSA in business + colloquial Najdi/Hejazi
    "SA": ExtendedLocale(
        "SR", True, ".", ",", 0.15, "VAT", "%d/%m/%Y", True, 0, "ar-SA", ("ar-SA", "en-SA")
    ),
    # Oman: Omani Arabic
    "OM": ExtendedLocale(
        "﷼", True, ".", ",", 0.05, "VAT", "%d/%m/%Y", True, 0, "ar-OM", ("ar-OM", "en-OM")
    ),
    # Qatar: Qatari Arabic
    "QA": ExtendedLocale(
        "QR", True, ".", ",", 0.0, "", "%d/%m/%Y", True, 0, "ar-QA", ("ar-QA", "en-QA")
    ),
    # Bahrain
    "BH": ExtendedLocale(
        "BD", True, ".", ",", 0.10, "VAT", "%d/%m/%Y", True, 0, "ar-BH", ("ar-BH", "en-BH")
    ),
    # Yemen: Yemeni Arabic
    "YE": ExtendedLocale("﷼", True, ".", ",", 0.0, "", "%d/%m/%Y", True, 6, "ar-YE", ("ar-YE",)),
    # Kuwait
    "KW": ExtendedLocale(
        "KD", True, ".", ",", 0.0, "", "%d/%m/%Y", True, 0, "ar-KW", ("ar-KW", "en-KW")
    ),
    # Jordan: Levantine Arabic (ar-JO) — distinct from Gulf Arabic
    "JO": ExtendedLocale(
        "JD", True, ".", ",", 0.16, "GST", "%d/%m/%Y", True, 0, "ar-JO", ("ar-JO", "en-JO")
    ),
    # Lebanon: Lebanese Arabic + French + English — genuinely trilingual in business
    "LB": ExtendedLocale(
        "LL", True, ",", ".", 0.11, "TVA", "%d/%m/%Y", True, 0, "ar-LB", ("ar-LB", "fr-LB", "en-LB")
    ),
    # Syria: Levantine Arabic
    "SY": ExtendedLocale("£S", True, ",", ".", 0.0, "", "%d/%m/%Y", True, 6, "ar-SY", ("ar-SY",)),
    ...
    "IQ": ExtendedLocale(
        "ID", True, ",", ".", 0.0, "", "%d/%m/%Y", True, 6, "ar-IQ", ("ar-IQ", "ku-IQ")
    ),
    ...
    "TR": ExtendedLocale(
        "₺", True, ",", ".", 0.20, "KDV", "%d.%m.%Y", False, 0, "tr-TR", ("tr-TR", "ku-TR")
    ),
    # ── South Asia ───────────────────────────────────────────────────────────
    ...
    # Bangladesh: Bengali
    "BD": ExtendedLocale(
        "৳", True, ".", ",", 0.15, "VAT", "%d/%m/%Y", False, 0, "bn-BD", ("bn-BD", "en-BD")
    ),
    ...
    "NP": ExtendedLocale(
        "₨", True, ".", ",", 0.13, "VAT", "%d/%m/%Y", False, 0, "ne-NP", ("ne-NP", "mai-NP")
    ),
    # Afghanistan: Pashto + Dari/Afghan Persian
    "AF": ExtendedLocale(
        "Af", True, ",", ".", 0.0, "", "%d/%m/%Y", True, 6, "ps-AF", ("ps-AF", "fa-AF")
    ),
    # ── East Asia ────────────────────────────────────────────────────────────
    # China: Mandarin Chinese — zh-CN is Simplified Chinese (mainland)
    "CN": ExtendedLocale(
        "¥", True, ".", ",", 0.13, "增值税", "%Y/%m/%d", False, 0, "zh-CN", ("zh-CN",)
    ),
    # Japan
    "JP": ExtendedLocale(
        "¥", True, ".", ",", 0.10, "消費税", "%Y/%m/%d", False, 0, "ja-JP", ("ja-JP",)
    ),
    # South Korea
    "KR": ExtendedLocale(
        "₩", True, ".", ",", 0.10, "부가가치세", "%Y.%m.%d", False, 0, "ko-KR", ("ko-KR",)
    ),
    # Hong Kong: Traditional Chinese (zh-HK) + English — genuinely bilingual in business
    "HK": ExtendedLocale(
        "HK$", True, ".", ",", 0.0, "", "%d/%m/%Y", False, 0, "zh-HK", ("zh-HK", "en-HK")
    ),
    # Macao: Traditional Chinese + Portuguese + Cantonese
    "MO": ExtendedLocale(
        "P", True, ".", ",", 0.0, "", "%d/%m/%Y", False, 0, "zh-MO", ("zh-MO", "pt-MO")
    ),
    # Taiwan: Traditional Chinese (zh-TW) — different from zh-CN
    "TW": ExtendedLocale(
        "NT$", True, ".", ",", 0.05, "營業稅", "%Y/%m/%d", False, 0, "zh-TW", ("zh-TW",)
    ),
    "MN": ExtendedLocale(
        "₮", True, ",", " ", 0.10, "VAT", "%Y-%m-%d", False, 0, "mn-MN", ("mn-MN", "ru-MN")
    ),
    "KP": ExtendedLocale("₩", True, ".", ",", 0.0, "", "%Y-%m-%d", False, 0, "ko-KP", ("ko-KP",)),
    # ── Southeast Asia ───────────────────────────────────────────────────────
    ...
    "TH": ExtendedLocale(
        "฿", True, ".", ",", 0.07, "VAT", "%d/%m/%Y", False, 0, "th-TH", ("th-TH", "en-TH")
    ),
    ...
    "VN": ExtendedLocale(
        "₫", False, ",", ".", 0.10, "VAT", "%d/%m/%Y", False, 0, "vi-VN", ("vi-VN",)
    ),
    # Myanmar: Burmese + English
    "MM": ExtendedLocale(
        "K", True, ".", ",", 0.05, "CT", "%d/%m/%Y", False, 0, "my-MM", ("my-MM", "en-MM")
    ),
    "KH": ExtendedLocale(
        "៛", False, ",", ".", 0.10, "VAT", "%d/%m/%Y", False, 0, "km-KH", ("km-KH", "en-KH")
    ),
    "LA": ExtendedLocale(
        "₭", True, ",", ".", 0.10, "VAT", "%d/%m/%Y", False, 0, "lo-LA", ("lo-LA",)
    ),
    # ── Central Asia ─────────────────────────────────────────────────────────
    # Kazakhstan: Kazakh + Russian (both official)
    "KZ": ExtendedLocale(
        "₸", False, ",", " ", 0.12, "ҚҚС", "%d.%m.%Y", False, 0, "kk-KZ", ("kk-KZ", "ru-KZ")
    ),
    "TM": ExtendedLocale(
        "T", True, ",", " ", 0.15, "VAT", "%d.%m.%Y", False, 0, "tk-TM", ("tk-TM", "ru-TM")
    ),
    "TJ": ExtendedLocale(
        "SM", True, ",", " ", 0.18, "VAT", "%d.%m.%Y", False, 0, "tg-TJ", ("tg-TJ", "ru-TJ")
    ),
    # Kyrgyzstan: Kyrgyz + Russian (both official)
    "KG": ExtendedLocale(
        "с", False, ",", " ", 0.12, "НДС", "%d.%m.%Y", False, 0, "ky-KG", ("ky-KG", "ru-KG")
    ),
    # Uzbekistan: Uzbek + Russian
    "UZ": ExtendedLocale(
        "сўм", False, ",", " ", 0.12, "QQS", "%d.%m.%Y", False, 0, "uz-UZ", ("uz-UZ", "ru-UZ")
    ),
    # Azerbaijan: Azerbaijani + Russian
    "AZ": ExtendedLocale(
        "₼", True, ",", ".", 0.18, "ƏDV", "%d.%m.%Y", False, 0, "az-AZ", ("az-AZ", "ru-AZ")
    ),
    "AM": ExtendedLocale(
        "֏", False, ",", " ", 0.20, "ԱԱՀ", "%d.%m.%Y", False, 0, "hy-AM", ("hy-AM", "ru-AM")
    ),
    ...
    "US": ExtendedLocale(
        "$", True, ".", ",", 0.0, "", "%m/%d/%Y", False, 6, "en-US", ("en-US", "es-US")
    ),
    # Canada: English + French (both official) — en-CA and fr-CA are both real
    "CA": ExtendedLocale(
        "CA$", True, ".", ",", 0.05, "GST", "%Y-%m-%d", False, 0, "en-CA", ("en-CA", "fr-CA")
    ),
    # Mexico: Spanish + indigenous (Nahuatl, Mayan, etc.)
    "MX": ExtendedLocale(
        "MX$", True, ".", ",", 0.16, "IVA", "%d/%m/%Y", False, 0, "es-MX", ("es-MX",)
    ),
    # ── Caribbean ────────────────────────────────────────────────────────────
    # T&T: English + Trinidadian Creole
    "TT": ExtendedLocale(
        "TT$", True, ".", ",", 0.125, "VAT", "%d/%m/%Y", False, 0, "en-TT", ("en-TT",)
    ),
    "JM": ExtendedLocale(
        "J$", True, ".", ",", 0.15, "GCT", "%d/%m/%Y", False, 0, "en-JM", ("en-JM",)
    ),
    "BB": ExtendedLocale(
        "Bds$", True, ".", ",", 0.175, "VAT", "%d/%m/%Y", False, 0, "en-BB", ("en-BB",)
    ),
    "VC": ExtendedLocale(
        "EC$", True, ".", ",", 0.15, "VAT", "%d/%m/%Y", False, 0, "en-VC", ("en-VC",)
    ),
    "LC": ExtendedLocale(
        "EC$", True, ".", ",", 0.15, "VAT", "%d/%m/%Y", False, 0, "en-LC", ("en-LC",)
    ),
    "GD": ExtendedLocale(
        "EC$", True, ".", ",", 0.15, "VAT", "%d/%m/%Y", False, 0, "en-GD", ("en-GD",)
    ),
    "DM": ExtendedLocale(
        "EC$", True, ".", ",", 0.15, "VAT", "%d/%m/%Y", False, 0, "en-DM", ("en-DM", "fr-DM")
    ),
    "AG": ExtendedLocale(
        "EC$", True, ".", ",", 0.15, "VAT", "%d/%m/%Y", False, 0, "en-AG", ("en-AG",)
    ),
    "KN": ExtendedLocale(
        "EC$", True, ".", ",", 0.17, "VAT", "%d/%m/%Y", False, 0, "en-KN", ("en-KN",)
    ),
    # Dominican Republic: Spanish only
    "DO": ExtendedLocale(
        "RD$", True, ".", ",", 0.18, "ITBIS", "%d/%m/%Y", False, 0, "es-DO", ("es-DO",)
    ),
    # Haiti: French + Haitian Creole (both official — Creole is dominant in daily life)
    "HT": ExtendedLocale(
        "G", True, ",", ".", 0.10, "TVA", "%d/%m/%Y", False, 0, "fr-HT", ("fr-HT", "ht-HT")
    ),
    "CU": ExtendedLocale("₱", True, ".", ",", 0.0, "", "%d/%m/%Y", False, 0, "es-CU", ("es-CU",)),
    # ── Central America ──────────────────────────────────────────────────────
    "NI": ExtendedLocale(
        "C$", True, ".", ",", 0.15, "IVA", "%d/%m/%Y", False, 0, "es-NI", ("es-NI",)
    ),
    "SV": ExtendedLocale(
        "$", True, ".", ",", 0.13, "IVA", "%d/%m/%Y", False, 0, "es-SV", ("es-SV",)
    ),
    "GT": ExtendedLocale(
        "Q", True, ".", ",", 0.12, "IVA", "%d/%m/%Y", False, 0, "es-GT", ("es-GT",)
    ),
    "HN": ExtendedLocale(
        "L", True, ".", ",", 0.15, "ISV", "%d/%m/%Y", False, 0, "es-HN", ("es-HN",)
    ),
    "CR": ExtendedLocale(
        "₡", True, ",", ".", 0.13, "IVA", "%d/%m/%Y", False, 0, "es-CR", ("es-CR",)
    ),
    "PA": ExtendedLocale(
        "B/.", True, ".", ",", 0.07, "ITBMS", "%d/%m/%Y", False, 0, "es-PA", ("es-PA", "en-PA")
    ),
    # ── South America ────────────────────────────────────────────────────────
    # Brazil: Portuguese — pt-BR is meaningfully different from pt-PT
    "BR": ExtendedLocale("R$", True, ",", ".", 0.0, "", "%d/%m/%Y", False, 0, "pt-BR", ("pt-BR",)),
    "AR": ExtendedLocale(
        "$", True, ",", ".", 0.21, "IVA", "%d/%m/%Y", False, 0, "es-AR", ("es-AR",)
    ),
    "CL": ExtendedLocale(
        "$", True, ",", ".", 0.19, "IVA", "%d/%m/%Y", False, 0, "es-CL", ("es-CL",)
    ),
    "CO": ExtendedLocale(
        "$", True, ",", ".", 0.19, "IVA", "%d/%m/%Y", False, 0, "es-CO", ("es-CO",)
    ),
    "PE": ExtendedLocale(
        "S/",
        True,
    ...
    "VE": ExtendedLocale(
        "Bs.D", True, ",", ".", 0.16, "IVA", "%d/%m/%Y", False, 0, "es-VE", ("es-VE",)
    ),
    "EC": ExtendedLocale(
        "$", True, ".", ",", 0.12, "IVA", "%d/%m/%Y", False, 0, "es-EC", ("es-EC", "qu-EC")
    ),
    ...
    "PY": ExtendedLocale(
        "₲", True, ",", ".", 0.10, "IVA", "%d/%m/%Y", False, 0, "es-PY", ("es-PY", "gn-PY")
    ),
    "UY": ExtendedLocale(
        "$", True, ",", ".", 0.22, "IVA", "%d/%m/%Y", False, 0, "es-UY", ("es-UY",)
    ),
    # ── Oceania ──────────────────────────────────────────────────────────────
    # Australia: English — en-AU is distinct (spelling, idioms, local tax terms)
    "AU": ExtendedLocale(
        "A$", True, ".", ",", 0.10, "GST", "%d/%m/%Y", False, 0, "en-AU", ("en-AU",)
    ),
    "NZ": ExtendedLocale(
        "NZ$", True, ".", ",", 0.15, "GST", "%d/%m/%Y", False, 0, "en-NZ", ("en-NZ", "mi-NZ")
    ),
    ...
    "TO": ExtendedLocale(
        "T$", True, ".", ",", 0.15, "VAT", "%d/%m/%Y", False, 0, "en-TO", ("en-TO", "to-TO")
    ),
    "WS": ExtendedLocale(
        "T", True, ".", ",", 0.15, "VAT", "%d/%m/%Y", False, 0, "sm-WS", ("sm-WS", "en-WS")
    ),
    "KI": ExtendedLocale(
        "A$", True, ".", ",", 0.0, "", "%d/%m/%Y", False, 0, "en-KI", ("en-KI", "gil-KI")
    ),
    "MH": ExtendedLocale(
        "$", True, ".", ",", 0.0, "", "%m/%d/%Y", False, 6, "en-MH", ("en-MH", "mh-MH")
    ),
    "FM": ExtendedLocale("$", True, ".", ",", 0.0, "", "%m/%d/%Y", False, 6, "en-FM", ("en-FM",)),
    "PW": ExtendedLocale("$", True, ".", ",", 0.0, "", "%m/%d/%Y", False, 6, "en-PW", ("en-PW",)),
    "SB": ExtendedLocale(
        "SI$", True, ".", ",", 0.15, "PPIP", "%d/%m/%Y", False, 0, "en-SB", ("en-SB",)
    ),
    ...
    "NR": ExtendedLocale(
        "A$", True, ".", ",", 0.0, "", "%d/%m/%Y", False, 0, "en-NR", ("en-NR", "na-NR")
    ),
    "TV": ExtendedLocale(
        "A$", True, ".", ",", 0.0, "", "%d/%m/%Y", False, 0, "en-TV", ("en-TV", "tvl-TV")
    ),
    "TK": ExtendedLocale(
        "NZ$", True, ".", ",", 0.0, "", "%d/%m/%Y", False, 0, "en-TK", ("en-TK", "tkl-TK")
    ),
    # fmt: on
}
